...
if __name__ == '__main__':
    ...
    with tf.device(device):
        ...
        face_feature = np.load('./data/face.npy')
        face_num = len(face_feature)
        ...
        dist_before = tf.reduce_sum(tf.multiply(diff, diff))
        dist = threshold-dist_before/face_num
        ...
        def f(image_inp): return persisted_sess.run(dist,
                                                    feed_dict={persisted_input: np.reshape(image_inp, (-1, 112, 112, 3)),
                                                               target_placeholder: np.reshape(face_feature, (-1, 512))
                                                               })
        ...
        def grad_b(image_inp): return persisted_sess.run(grad, feed_dict={persisted_input: np.reshape(image_inp, (-1, 112, 112, 3)),
                                                                          target_placeholder: np.reshape(face_feature,
                                                                                                         (-1, 512))})
        ...
        v = targeted_perturbation(X, f, get_f, grad_b, delta=0.3,max_iter_uni=10,target=target,p=2, xi=3.9)
